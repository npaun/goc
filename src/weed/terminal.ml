...
| [] -> []
| [last] when mustreturn -> [{last with v = force_return mode last.v}]
...
in match block with
| [] when mustreturn -> raise (SyntaxError "This function must return a result but this block is empty")
| any -> aux any

and pass_statement (mustreturn,maybreak,maycontinue) = function
| Break when not maybreak -> raise (SyntaxError "break must only be used in a non-terminal loop")
...
| Switch(stmt, expr, fallcases) -> Switch(stmt,expr, List.map (pass_fallablecase (false,true,true)) fallcases)
...
| Return _ -> raise (SyntaxError "Unexpected return - this is not the terminal statement")
| any -> any
...
and pass_switch cases = 
	let has_default = List.exists (function | ((Default _),_)  -> true | _ -> false) cases in
		if has_default then
			List.map (pass_fallablecase (true,false,true)) cases
		else
